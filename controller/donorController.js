...
const getDonors = async (req, res, next) => {
  try {
    ...
    const donors = await DonorProfile.find(filter)
      .populate({
        path: "user",
        select: "firstname surname bloodType location phone",
      })
      .skip((page - 1) * limit)
...
const acceptRequest = async (req, res, next) => {
  try {
    ...
    if (!donorProfile)
      return res.status(404).json({ message: "Donor profile not found" });
    if (!donorProfile.availabilityStatus)
      return res.status(400).json({ message: "You are unavailable" });
    ...
    if (!request)
      return res
        .status(400)
        .json({ message: "Request already taken or invalid" });
    const compatibleDonorTypes = getCompatibleDonorTypes(
  ...
  try {
    ...
    if (!donorBloodType || !donorLocation?.coordinates?.length)
      return res
        .status(400)
        .json({
          message: "Complete your profile first (blood type + location)",
        });
    ...
    const nearbyRequests = await PatientRequest.find({
      ...
      rejectedBy: { $ne: req.user.id },
      location: {
        $near: { $geometry: donorLocation, $maxDistance: maxDistanceMeters },
      },
    }).populate("patient", "firstname surname bloodType phone");
    const compatibleRequests = nearbyRequests.filter((request) => {
      return getCompatibleDonorTypes(request.requiredBloodType).includes(
        donorBloodType,
      );
    });
...
const confirmDonation = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== "bloodbank" && req.user.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Only blood banks can confirm donations" });
    }
